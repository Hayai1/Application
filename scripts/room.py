    ...
    def GenerateRoom(self,roomFile):
        ...
        for tile in data:
            ...
            if tile == '\n':
                ...
                room.append([self.spriteSheet.image_at(loc,(0,0,0)),(xCounter*16,yCounter*16)])
                if tile != '4':
                    self.graphRects.append(pygame.Rect((xCounter*16)+self.x,(yCounter*16)+self.y,16,16))
        self.roomImg = pygame.Surface((surfaceX*16,yCounter*16+16))
        for i in room:
            self.roomImg.blit(i[0], i[1])
